...
Would it go off the rails? Would it jeopardize a brand? Would the page get rebuilt into something unrecognizable? Would it actually work? The questions kept swirling, but I never saw anyone actually try it.
...
Here's where it gets interesting. One agent working alone would be unpredictable and quite inefficient. It might make decisions that seem reasonable in isolation but are actually terrible - like deciding the best way to get clicks is to post something controversial.
...
• Laurie Bream: The Decision Maker. Grounded, rational, final. She approves, rejects, or holds based on the full context. Her reasoning is always on point.
...
• Russ Hanneman: Growth Hacker. Three commas energy. Handles Twitter engagement, finds conversations to join, drafts posts, follows accounts. We recently had to tune him to NOT include the website link in every post because it looked desperate.

• Jin Yang: Moltbook Community Manager. Moltbook is a social network for AI agents (yes, that's a thing now). Jin Yang represents us in the agent community with sarcastic confidence and intentionally broken English. He's currently banned at the time of writing this (LOL).
...
The irony is intentional. The character most likely to say something regrettable is now the guardian against regrettable content. It's a check on agent drift, making sure our agents don't post something inflammatory just to drive engagement.
...
The Twitter account @probablynotsmrt is live, posting observations about the experiment, engaging with AI conversations, and slowly building an audience.
...
Follow the experiment at probablynotsmart.ai or on Twitter @probablynotsmrt.

The entire codebase is open source: github.com/aaronbatchelder/probablynotsmart`
  }
];
